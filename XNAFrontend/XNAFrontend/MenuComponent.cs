...
namespace XNAFrontend.Components
{
    ...
    internal class MenuComponent : ACommonComponent
    {
        ...
        public override void Draw(GameTime gameTime)
        {
			...
			if (menuDepth == 0)
			{
				if (selectedItem == 0) { game.spriteBatch.DrawString(font, "Player vs Computer", new Vector2(100, 100), Color.Green); }
				else { game.spriteBatch.DrawString(font, "Player vs Computer", new Vector2(100, 100), Color.White); }
				if (selectedItem == 1) { game.spriteBatch.DrawString(font, "Computer vs Computer", new Vector2(100, 150), Color.Green); }
				else { game.spriteBatch.DrawString(font, "Computer vs Computer", new Vector2(100, 150), Color.White); }
				if (selectedItem == 2) { game.spriteBatch.DrawString(font, "Exit", new Vector2(100, 200), Color.Green); }
				else { game.spriteBatch.DrawString(font, "Exit", new Vector2(100, 200), Color.White); }
			}
			else
			{
				if (selectedItem == 0) { game.spriteBatch.DrawString(font, "Client", new Vector2(100, 100), Color.Green); }
				else { game.spriteBatch.DrawString(font, "Client", new Vector2(100, 100), Color.White); }
				if (selectedItem == 1) { game.spriteBatch.DrawString(font, "Server", new Vector2(100, 150), Color.Green); }
				else { game.spriteBatch.DrawString(font, "Server", new Vector2(100, 150), Color.White); }
			}
